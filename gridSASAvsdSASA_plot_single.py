...
grid_nc = '%s/new_sasa.nc'%grid_path
...
bsite_file = None
grid_nc_file = f"{ppi_path}/2.redock/4.receptor_grid/2OOB_A:B/new_sasa.nc"
...
for i,rot_crd in enumerate(lig_rot):
    ...
    ls_fft = np.fft.fftn(lgrid["sasa"])
    lone_fft = np.fft.fftn(l_ones)
    rs_fft = np.fft.fftn(sasa)
    ...
    for v,vector in enumerate(translations):
        ...
        elif rmsd_to_native < 20 and rmsd_to_native >= 15:
            colors.append("blue")
        else:
...
name = f'/home/jim/src/p39/fftanalysis/plots/gridSASAvsdSASA/2OOB_A_200.jpg'
import pickle as p
values = {}
values['translations'] = all_translations
values['trans_scores'] = all_trans_scores
values['delta_sasas'] = all_delta_sasas
values['colors'] = all_colors
p.dump(values, open(f"{name[:-4]}_200.p", "wb"))

regplot(all_trans_scores, all_delta_sasas, all_colors, name)
